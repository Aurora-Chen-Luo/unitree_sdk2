  ...
  void Control() {
    ...
    if (ms) {
      ...
      if (time_ < duration_) {
        ...
        const double t = std::fmod(time_ - duration_, duration_ * 4.0);
        const double phase_t = std::fmod(t, duration_);
        const double max_angle = M_PI * 30.0 / 180.0;
        const double angle_des = max_angle * std::sin(2.0 * M_PI * phase_t / duration_);
        mode_pr_ = 0;

        if (t < duration_) {
          // [Stage 2]: swing wrist roll joints for 2 s
          motor_command_tmp.q_target.at(LeftWristRoll) = angle_des;
          motor_command_tmp.q_target.at(RightWristRoll) = -angle_des;
        } else if (t < duration_ * 2.0) {
          // [Stage 3]: swing wrist pitch joints for 2 s
          motor_command_tmp.q_target.at(LeftWristPitch) = angle_des;
          motor_command_tmp.q_target.at(RightWristPitch) = -angle_des;
        } else if (t < duration_ * 3.0) {
          // [Stage 4]: swing wrist yaw joints for 2 s
          motor_command_tmp.q_target.at(LeftWristYaw) = angle_des;
          motor_command_tmp.q_target.at(RightWristYaw) = -angle_des;
        } else {
          // [Stage 5]: swing head yaw joint for 2 s
          motor_command_tmp.q_target.at(HEAD_YAW) = angle_des;
        }
      }
